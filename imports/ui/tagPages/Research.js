...
render() {
    return (
      <div>
        {this.state.users ? <TagPageLayout users={this.state.users} latestCollection={latest} popularCollection={popular} creatorsPop={this.returnCreators('pop')} creatorsNew={this.returnCreators('new')} creatorsOld={this.returnCreators('old')} tag='Research' tagDescription="were we wrong?" relatedCategories={['Future', 'Technology', 'Economy']} categoryLinks={['/future', '/technology', '/economy']} relatedTags={['Innovation', 'Energy', 'Biodiversity']} tagLinks={['/innovation', '/energy', '/biodiversity']}  /> : undefined }
      </div>
    );
  }
}
