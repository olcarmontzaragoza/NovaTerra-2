...
render() {
    return (
      <div>
        {this.state.users ? <TagPageLayout users={this.state.users} latestCollection={this.returnLatest()} popularCollection={this.returnPopular()} creatorsPop={this.returnCreators('pop')} creatorsNew={this.returnCreators('new')} creatorsOld={this.returnCreators('old')} tag="Waste" tagDescription="how do we redefine waste?" relatedCategories={['Environment', 'Economy', 'Future']} categoryLinks={['/environment', '/economy', '/future']} relatedTags={['Food', 'Cities', 'Work']} tagLinks={['/food', '/cities', '/work']}  /> : undefined }
      </div>
    );
  }
}
