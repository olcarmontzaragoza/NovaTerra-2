...
render() {
    return (
      <div>
        {this.state.users ? <TagPageLayout users={this.state.users} latestCollection={this.returnLatest()} popularCollection={this.returnPopular()} creatorsPop={this.returnCreators('pop')} creatorsNew={this.returnCreators('new')} creatorsOld={this.returnCreators('old')} tag='Work' tagDescription="where does it from here?" relatedCategories={['Economy', 'Future', 'Now']} categoryLinks={['/economy', '/future', '/now']} relatedTags={['Personal Finance', 'Self', 'Cities']} tagLinks={['/personal-finance', '/self', '/cities']}  /> : undefined }
      </div>
    );
  }
}
