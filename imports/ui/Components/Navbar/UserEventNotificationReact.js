  ...
  render() {
    return (
      <div className={`nav__notificationsTopPaddingReact ${!this.props.notification.seen ? 'nav__notificationNotSeenBackground' : ''}`}>
          ...
          <a onClick={() => this.changePage(this.props.notification.postUrl)} className="nav__notificationsPostImagePositioningReact"><Image className="nav__notificationsStoryImageReact" cloud_name='novaterra' publicId={this.props.notification.postImage}><Transformation crop="thumb" /></Image></a>
          <div className="nav__notificationsTextNormalUserReact"><Link to={this.findUserAction().profileUrl} className="link">{this.findUserAction().username}</Link><a className="">&nbsp;{this.props.notification.description}<FontAwesomeIcon icon={['far', this.getReactType(this.props.notification.reactType)]} className='nav__notificationReactType' />.</a>
