...
render() {
    return (
      <div>
        ...
        <div className="mpl__mainAuthor floatLeft">{this.props.user.username}</div> {this.props.route && this.props.user._id !== Meteor.userId() ? <div>{this.state.profileFollow ? <button className="mpl__followingButtonLargeTop floatLeft" onClick={() => { this.toggleIsFollowing() }}>Following</button> : <button className="mpl__followButtonLarge floatLeft" onClick={() => { this.toggleIsFollowing() }}>Follow</button>}</div> : undefined }
        <div className="clearBoth"></div>
